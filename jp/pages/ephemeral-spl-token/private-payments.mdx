...
プライベート送金のコストは、互いに独立した 2 つの要素で決まります。**プライバシー手数料**（常に発生）と **gas の支払い方法**（自己負担 vs. gasless）です。

すべてのプライベート base → base 送金には **0.1%（10 bps）のプライバシー手数料**がかかり、送金対象の token で徴収されます — SOL ではありません。その上で、gas の支払い方法は 2 通りあります:

|  | 自己負担 | Gasless（`gasless: true`） |
| --- | --- | --- |
| SOL トランザクション手数料 | 送信者が支払う（SOL が必要） | sponsor が支払う（送信者は SOL 不要） |
| Relay 手数料 | なし | sponsor へ固定 0.2 USDC/USDT |
| 0.1% プライバシー手数料 | あり、token で徴収 | あり、token で徴収 |
| 最低金額 | なし | 0.5 USDC/USDT |
| 対応 mint | 任意 | mainnet USDC/USDT、devnet USDC |

`gasless: true` の場合、設定済みの sponsor がトランザクションの fee payer となって共同署名し、API は送信者の残高から sponsor に固定 **0.2 USDC/USDT の relay 手数料**を支払う token 送金 instruction をトランザクションの先頭に追加します。relay 手数料は固定額のため、gasless 送金には **0.5 USDC/USDT** の最低金額が設定されています — それ未満では手数料が送金額を食いつぶしてしまうためです。
...
<Note>
  0.5 の最低金額が制限するのは gasless オプションだけで、プライバシーの制限では**ありません**。
  それより小さい金額では `visibility: "private"` を維持したまま `gasless` を省略してください。
  送金は引き続きプライベートにルーティングされ、送信者は自分の SOL 手数料と 0.1% の
  プライバシー手数料を支払います。
</Note>

<Warning>
  0.5 の最低金額未満では、`gasless: true` のリクエストは `400 INVALID_GASLESS_TRANSFER_AMOUNT`
  エラーで失敗します — API がプライベート送金を黙って public に降格することはありません。
  クライアントが失敗した gasless リクエストをリトライする場合は、`visibility: "private"` を
  維持したままリトライするようにしてください。そうしないと少額送金が公開されてしまいます。
</Warning>

`from` が off-curve の PDA owner の場合、`gasless: true` は無視されます（gasless にはウォレット送信者が必要です）。その場合も送金はプライベートに実行され、送信者が fee payer になります。
