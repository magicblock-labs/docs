...
<Tabs>
    ...
    <Tab title="2. 委任と permission の作成">
        ...
        - `delegate` は counter のみを TEE バリデーターへ委任します（Base Layer の permission CPI は不要）。
        ...
        - `undelegate` は `MagicIntentBundleBuilder` 経由で counter をコミットし委任解除します — permission の委任解除手順は不要です。
