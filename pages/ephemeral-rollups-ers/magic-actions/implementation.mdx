...
<Note>
  - Use `discriminator = &EXTERNAL_CALL_HANDLER_DISCRIMINATOR` - Use
  `UncheckedAccount` for committed accounts and manual deserialization - First
  two accounts are injected: `escrow`, `escrow_auth`
</Note>
