...
| :--- | ---: | :--- |
| Normal ER transaction | `0` by default on the current public-node release | When you send the transaction |
...
Solana transaction fees are separate from the prices above. A validator can also configure its own
base ER transaction fee, even though the current public-node release defaults to zero.
