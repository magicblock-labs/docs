...
pub fn process_delegate(
    ...
    // Prepare counter pda seeds (Choose your own but make sure it's the same as for undelegating)
...
pub fn process_undelegate(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    pda_seeds: Vec<Vec<u8>> // Make sure it's the same as for delegating
) -> ProgramResult {
    // Get accounts
    let account_info_iter = &mut accounts.iter();
