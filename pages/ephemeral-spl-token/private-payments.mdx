...
Two independent choices determine what a private transfer costs: the **privacy fee** (always applies)
and **how gas is paid** (self-paid vs. gasless).

Every private base → base transfer pays a **0.1% (10 bps) privacy fee**, charged in the token being
transferred — not in SOL. On top of that, gas can be paid two ways:
...
minimum of **0.5 USDC/USDT** — below that the fee would swallow the transfer.
...
<Note>
  The 0.5 minimum gates only the gasless option — it is **not** a privacy limit. For smaller amounts,
  keep `visibility: "private"` and omit `gasless`: the transfer still routes privately, and the sender
  pays their own SOL fees plus the 0.1% privacy fee.
</Note>

<Warning>
  Below the 0.5 minimum, a `gasless: true` request fails with a `400 INVALID_GASLESS_TRANSFER_AMOUNT`
  error — the API never silently downgrades a private transfer to public. If your client retries a
  failed gasless request, make sure it retries with `visibility: "private"` intact, or small transfers
  will leak publicly.
</Warning>

`gasless: true` is ignored when `from` is an off-curve PDA owner (gasless requires a wallet sender);
the transfer still executes privately with the sender as fee payer.
