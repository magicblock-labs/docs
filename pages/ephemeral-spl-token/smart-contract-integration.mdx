...
#### 5. Commit & undelegate {#5-commit-undelegate}
...
<Tabs>
  ...
  <Tab title="Base-layer post-commit payout">
    ...
    <Warning>
      ...
      transfer with your PDA, it **must authenticate the caller** or it becomes an
      open drain on your token accounts. Require the injected `escrow` account as
      a `signer` and pin it to `ephemeral_balance_pda_from_payer(escrow_auth, 255)`,
      and bind `escrow_auth` to your paying PDA. See
