...
<Tabs>
    ...
    <Tab title="5. Authorize">
        ...
        1. Verify integrity of TEE RPC with `verifyTeeRpcIntegrity`

            a) Get quote: `https://tee.magicblock.app/quote?challenge=${challenge}`

            b) Verify quote: `https://pccs.phala.network/tdx/certification/v4`
        2. Request an authorization token for user to interact with TEE endpoint with `getAuthToken`:

            a) Request challenge: `https://tee.magicblock.app/auth/challenge?pubkey=${pubkey}`

            b) Post authorization with signature: `https://tee.magicblock.app/auth/login`
        <AuthorizeCode />
        [⬆️ Back to Top](#code-snippets)
    </Tab>
    <Tab title="6. Test">
