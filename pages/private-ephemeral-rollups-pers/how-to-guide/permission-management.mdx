...
import PermissionCards from "/snippets/permission-cards.mdx";
...
Permission management in Private Ephemeral Rollups enables fine-grained access control over permissioned accounts. Each permission account maintains a list of members with specific flags that determine what actions they can perform.
...
<Tabs>
  <Tab title="1. Create">
    Create a new permission account with initial members and their flags.
    ...
    **Use Cases:**
    ...
    [⬆️ Back to Top](#permission-operations)

  </Tab>

  <Tab title="2. Update">
    Modify existing members or their flags in a permission account.
  ...
  <Tab title="3. Delegate">
    Delegate a permissioned account to enable low-latency Private Ephemeral Rollup execution.
  ...
  <Tab title="4. Request">
    ...
    **Authorization Steps:**
    ...
    For detailed implementation, see [Client Implementation](/pages/private-ephemeral-rollups-pers/how-to-guide/client-implementation).
  ...
  <Tab title="5. Commit & Undelegate">
    Sync the final state to Solana and return the account to Base Layer control.
    ...
    **Use Cases:**
    ...
    - End an ER session and return control to Base Layer
    - Finalize all pending state changes
    - Prepare account for closing or resuming normal operations
  ...
  <Tab title="6. Close">
    Close a permissioned account and reclaim its lamports.
    ...
    **Use Cases:**
    ...
    - Remove access control when no longer needed
