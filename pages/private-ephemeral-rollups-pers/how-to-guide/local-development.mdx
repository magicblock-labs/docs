...
<Steps>
  <Step title="Install the binaries">
    `ephemeral-validator` and `mb-test-validator` ship together:

    ```bash
    npm install -g @magicblock-labs/ephemeral-validator@latest
    ```

    `query-filtering-service` (and `vrf-oracle`, if you need VRF) must also be on
    your `PATH` — they ship with the MagicBlock validator tooling. Verify:

    ```bash
    command -v ephemeral-validator query-filtering-service
    ```
  ...
  <Step title="Start the base validator (port 8899)">
    ...
    Wait until it is actually **producing slots** before starting the ER — poll
    `getSlot` until it returns a value greater than `0`. The ER's account cloner
    fails to bootstrap if it starts against a bank that isn't producing yet.

    <Note>
      `--reset` airdrops the CLI wallet ~500M SOL, which exceeds the ER's
      internal faucet. If you use that wallet as an ER fee payer, the cloner
      fails with `InsufficientFunds`. Drain it down to ~1000 SOL first (transfer
      the excess to the incinerator `1nc1nerator11111111111111111111111111111111`).
    </Note>
  ...
  <Step title="Start the ephemeral validator (port 7799)">
    ...
    Wait for port `7799` to accept a TCP connection before continuing.
...
The [`magicblock-engine-examples`](https://github.com/magicblock-labs/magicblock-engine-examples)
repo wires all of this up. To bring the full cluster up and leave it running for
manual testing:

```bash
# from the repo root
SETUP_ONLY=1 ./scripts/test-locally.sh
```

Then, in another shell, source the shared endpoints and run one example's tests
...
Per example, `yarn setup` brings the stack up and `yarn test:local` runs the
tests against it.
