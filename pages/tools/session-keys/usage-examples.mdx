...
2. `topUp`: A boolean value, set to `true` if you want to top up an session keypair with `0.01 SOL` initially and \`false\` if you don't want to topUp the session Keypair.
